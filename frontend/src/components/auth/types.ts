...
export interface BaseStepProps<TFormData, TStep extends string> {
  formData: TFormData;
  onUpdateData: (field: keyof TFormData, value: any) => void;
  isLoading: boolean;
...
export interface RecaptchaV2StepProps01<TFormData, TStep extends string, TResponse>
  extends StepWithPrevious<TFormData, TStep> {
  onVerify: (v2Token: string) => Promise<ApiResult<TResponse>>;
}

// 이메일 코드 검증을 위한 스텝
export interface VerificationStepProps01<TFormData, TStep extends string>
  extends StepWithPrevious<TFormData, TStep> {
  onComplete: () => void;
}
