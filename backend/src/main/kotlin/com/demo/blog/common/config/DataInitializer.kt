...
import com.demo.blog.user.domain.Role
    ...
    fun initRoles() {
        try {
            if (roleRepository.findByName("ROLE_USER") == null) {
                roleRepository.save(Role(name = "ROLE_USER", description = "일반 사용자"))
                logger.info { "ROLE_USER 역할이 생성되었습니다." }
            }
            
            if (roleRepository.findByName("ROLE_ADMIN") == null) {
                roleRepository.save(Role(name = "ROLE_ADMIN", description = "시스템 관리자"))
